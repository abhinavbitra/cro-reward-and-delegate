...
CHAINID=crossfire
COUNT=20 #Number of transactions till check of last transaction
SLEEP=1s #length of the sleep before the scrip tries to check if the last transaction was broadcasted (0 = disabled)
...
then
 ...
 then
  ...
  ./chain-maind tx distribution set-withdraw-addr $ADDRESS --from $ADDRESS --chain-id $CHAINID --gas-prices="0.5basetcro" --gas 80000000 --generate-only > tx.json
