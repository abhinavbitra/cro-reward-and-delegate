...
do
    ...
    if (( $(echo "$currentAvailableReward > 1000" |bc -l) )) 
    then
            echo "Withdrawing rewards..."
            echo $keyPassword | ./chain-maind tx distribution withdraw-rewards $validatorAddress --commission --from $keyring --gas 800000000 --gas-prices 0.1basetcro --chain-id="crossfire"  --node $node  -y
    fi
    if (( $(echo "$currentBalance > 1000" |bc -l) )) 
    then
            echo "Re-delegating rewards..."
            echo $keyPassword | ./chain-maind tx staking delegate $validatorAddress 0.001basetcro --from $keyring --gas 800000000 --gas-prices 0.1basetcro --chain-id="crossfire"  --node $node  -y
    fi
    sleep 4m
done
