...
then
    ...
    echo "./reward-and-delegate-nokey-p.sh <operatorAddress> <validatorAddress> <keyPassword>"
    exit 0
fi
...
do
    ...
    then
            ...
            echo $keyPassword | ./chain-maind tx staking delegate $validatorAddress 0.001tcro --from cross-fire-testing --gas 80000000 --gas-prices 0.1basetcro --chain-id "crossfire" --node "https://crossfire.crypto.com:443"  -y
    fi
    sleep 2s
done
