...
do
    ...
    then
            ...
            echo $keyPassword | ./chain-maind tx staking delegate $validatorAddress "0.01"basetcro --from $keyring --gas 80000000 --gas-prices 0.1basetcro --chain-id="crossfire" --node $node  -y
    fi
    sleep 3s
done
