...
do
    ...
    then
            ...
            echo $keyPassword | ./chain-maind tx distribution withdraw-rewards $validatorAddress --commission --from cross-fire-testing --gas 8000000 --gas-prices 0.1basetcro --chain-id="crossfire" --node $node  -y
    fi
    if (( $(echo "$currentBalance > 10000" |bc -l) )) 
    then
            echo "Re-delegating rewards..."
            echo $keyPassword | ./chain-maind tx staking delegate $validatorAddress 0.01tcro --from cross-fire-testing --gas 8000000 --gas-prices 0.1basetcro --chain-id="crossfire" --node $node  -y
